...
let mediaScanUrl = '/vediter-scan-media';
...
class MediaModal {
	...
	save() {
		...
		if (file.indexOf("//") == -1) {
			src = '/storage/'+this.mediaPath + file;
		}
...
_
        ...
        onUpload(event) {
            ...
            if (event.target.files && event.target.files[0]) {
                ...
                file = event.target.files[0]; // Define the file variable here
            ...
            function imageIsLoaded(e) {
                ...
                let currentPath = Vvveb.MediaModal.currentPath;

                // Check if currentPath is not empty, then remove the prefix 'storage/veditor/media/'
                if (currentPath) {
                    currentPath = currentPath.replace('storage/veditor/media/', '');
                }

                // Append the modified currentPath to formData
                formData.append("mediaPath", Vvveb.MediaModal.mediaPath + currentPath);
                formData.append("onlyFilename", true);

                // Get the CSRF token from the meta tag
                let token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

                fetch('/vediter-upload-media', {
                    method: "POST",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
                ...
                .then(data => {
                    ...
                    displayToast("bg-success", "Success", "File uploaded successfully!");
                })
                .catch(error => {
                    console.log(error);
                    Vvveb.MediaModal.hideUploadLoading();
                    displayToast("bg-danger", "Error", error.message || "Error uploading!");
                });
            }
        }
		...
		deleteFile(el) {
            let deleteUrl='/vediter-delete-media'
			let parent = el.closest("li");
			let file = parent.querySelector('input[type ="hidden"]').value;
            let formDataDelete = new FormData();
          let  file2=file.replace('/storage/veditor/media/', 'veditor/media/');

			if (confirm(`Are you sure you want to delete "${file2}"template?`)) {
                formDataDelete.append('mediaPath',file2);
                let token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
			fetch(deleteUrl, {
                method: "POST",
                headers: {
				...
				.then((data) => {
                    ...
                    displayToast(bg,data.success?'Success':'Error', data.message);
        ...
        renameFile(el) {
            let deleteUrl='/vediter-rename-media'
			let parent = el.closest("li");
			let file = parent.querySelector('input[type ="hidden"]').value;
            let formDataDelete = new FormData();
          ...
          let  file2=file.replace('/storage/veditor/media/', 'veditor/media/');
			...
			if (confirm(`Are you sure you want to Rename "${file2}"template?`)) {
                let newfile = prompt(`Enter new file name for "${file2}"`);
                formDataDelete.append('newName',newfile);
                formDataDelete.append('mediaPath',file2);
                let token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
			fetch(deleteUrl, {
                method: "POST",
		...
		addFile(f, selected) {
				...
				let fileSize = _this.bytesToSize(f.size),
					...
					if (fileType == "jpg" || fileType == "jpeg" || fileType == "png" || fileType == "gif" || fileType == "svg" || fileType == "webp") {
						...
						icon = '<img class="image" loading="lazy" src="/storage/' + _this.mediaPath + f.path + '">';
						isImage = true;
					} else {
				...
				actions += ' <a href="javascript:void(0);" title="Delete" class="btn btn-outline-danger btn-sm border-0 btn-delete"><i class="la la-trash"></i></a>';
				...
				let file = generateElements('<li class="files">\
						...
						<input type="hidden" value="/storage/' +  _this.mediaPath + f.path + '" name="filename[]">\
						  ...
						  <div href="#\" class="files">'+icon+'<div class="info"><div class="name">'+ name +'</div><span class="details">'+fileSize+'</span>\
							' + actions + '\
							 <div class="preview">\
								<img src="/storage/' + _this.mediaPath + f.path + '">\
								<div>\
